...
var columnTemplate= {
  "manufactureOrderCoreNo": function (d) {
    switch (d.manufactureOrderCoreNo.trim()) {
      case "":
        return "<button class='btn grid' onclick='location=\"/page/manufacture/order/form/" + d.clothOrderDetailUuid + "\"'>製令</button>";
      default:
        return "<a class='number_a' href='/page/manufacture/order/form/" + d.clothOrderDetailUuid + "'>" + d.manufactureOrderCoreNo + "</a>";
    }
  },
  "deliverDateAndQuantity": function (d) {
    if(!d.deliverDateAndQuantity[0]){
      return "";
    }
    return d.deliverDateAndQuantity[0];
  }
}
var extraColumns = [
  {
    title: "交期/下單量",
    template: function (d) {
      if(!d.deliverDateAndQuantity[1]){
      return "";
      }
      return d.deliverDateAndQuantity[1];
    }
  }
]
var dataSource = new kendo.data.DataSource({
  transport: {
    read: {
      url: '/api/manufacture/order/grid',
      dataType: "json",
      type: "POST",
      processData: false,
      contentType: false
