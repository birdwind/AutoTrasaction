...
var purchaselistAjAX = $.ajax({
  url: "/api/machine/weaving/purchase/company/list",
  method: "GET",
...
var subordinateAjAX = $.ajax({
  url: "/api/machine/weaving/subordinate/company/list",
  method: "GET",
  success: function (data) {
...
var arealist = {};
var areaAjAX = $.ajax({
  url: "/api/machine/weaving/station/area/list",
  method: "GET",
  success: function (data) {
    arealist = data.response;
  } // end of ajax success
}); //end of ajax
var specificationlist = {};
var specificationAjAX = $.ajax({
  url: "/api/machine/weaving/beam/specification/list",
  method: "GET",
  success: function (data) {
    specificationlist = data.response;
  } // end of ajax success
}); //end of ajax
var selectedValue = {};
...
$(async function () {
  ...
  var grid = new Grid("/machine/weaving/", "onlyEditColumn,check");
  grid.setSortField("purchaseDate", "desc");
  var kendoData = {
    id: "weavingMachineCoreNo",
    fields: {
      weavingMachineCoreNo: {
        nullable: false,
      },
      weavingMachineSpec: {
        type: "string"
  ...
  let saveData = {
    url: "/api/machine/weaving/",
    saveEvent: function (e) {
      e.preventDefault();
      let theGrid = $("#weaving").data("kendoGrid");
      var postData = new FormData();
      var data = new Object();
      let beamWidth = "";
      let beamSpecification = [];

      (selectedValue.beamSpecificationValues) ? selectedValue.beamSpecificationValues: selectedValue.beamSpecificationValues = theGrid.dataItem(theGrid.select()).beamSpecificationValues;
      (theGrid.dataItem(theGrid.select()).beamWidth == "寬") ? beamWidth = 1: beamWidth = 0;

      $.each(selectedValue.beamSpecificationValues, function (i, j) {
        if (specificationlist.find(element => element.text == j)) beamSpecification.push(specificationlist.find(element => element.text == j).value);
      })
      console.log(beamSpecification);
      data.weavingMachineCoreUuid = (theGrid.dataItem(theGrid.select()).weavingMachineCoreUuid) ? theGrid.dataItem(theGrid.select()).weavingMachineCoreUuid.trim() : 0;
      data.weavingMachineCoreNo = (theGrid.dataItem(theGrid.select()).weavingMachineCoreNo) ? theGrid.dataItem(theGrid.select()).weavingMachineCoreNo.trim() : 0;
      data.weavingMachineSpec = (theGrid.dataItem(theGrid.select()).weavingMachineSpec) ? theGrid.dataItem(theGrid.select()).weavingMachineSpec : 0;
      ...
      data.locateAreaValue = (theGrid.dataItem(theGrid.select()).locateAreaValue) ? theGrid.dataItem(theGrid.select()).locateAreaValue : 0;
      data.weftMaxCapacity = (theGrid.dataItem(theGrid.select()).weftMaxCapacity) ? theGrid.dataItem(theGrid.select()).weftMaxCapacity : 0;
      data.beamWidth = beamWidth;
      data.isDobby = (theGrid.dataItem(theGrid.select()).isDobby);
      data.beamSpecificationValues = beamSpecification;
      data.dobbyAmount = (theGrid.dataItem(theGrid.select()).dobbyAmount) ? theGrid.dataItem(theGrid.select()).dobbyAmount : 0;

      postData.append("weavingMachineCore", new Blob([JSON.stringify(data)], {
      ...
      $.ajax({
        url: "/api/machine/weaving/",
        data: postData,
        method: (data.weavingMachineCoreUuid) ? "POST" : "PUT",
        contentType: false,
        processData: false,
        dataType: "json",
        success: function (data) {
  ...
  grid.setTemplate("beamSpecificationValues", "gridMultiSelect", "", "", specificationlist, "#= (beamSpecificationValues)?beamSpecificationValues.join(','):''#");
  grid.setTemplate("locateAreaValue", "gridDropDown", "", "", arealist, );
  grid.setTemplate("beamWidth", "gridEditDropDown", "", "", "", );


  grid.setTemplate("purchaseDate", "date");
  grid.setTemplate("isDobby", "checkColumn", "", "", "", );
  let filterable = {
    mode: "row",
    messages: {
      info: ""
  ...
  await grid.creatKendGrid("#weaving", filterable, edit, false);
  $('body').on('click', '#weaving .k-checkbox ', function () {
    $("#trashBin").hide();
    if ($("#weaving .k-grid-content tbody input:checkbox:checked").length - $("#weaving .k-grid-content tbody .ingrid:checkbox:checked").length > 0) {
      $("#trashBin").show();
    }

  }) //checkbox 
  $("#trashBin").click(function () {
    if (!$("#weaving .k-grid-content tbody input:checkbox:checked").length) {
      return false;
    }
    fw_confirmBox.show();
  })
  ...
  $('body').on('click', '#addbtn', function () {
    var grid = $("#weaving").data("kendoGrid");
    grid.addRow();
    var row = $("#weaving").find(".k-grid-edit-row");
    grid.select(row);
    ...
    row.find(".k-multiselect").addClass("form-control");
    row.find(".k-multiselect").addClass("form-control");

    $(".k-multiselect").css("height", "auto");
    row.find(".k-multiselect").css("width", "96%");
    row.find(".k-checkbox-label").hide();
    row.find$(".ingrid").show();
...
function confirmDel() {
  ...
  var uuidSet = [];
  var theGrid = $('#weaving').data("kendoGrid");

  var postData = new FormData();
  theGrid.select().each(function () {
    uuidSet.push(theGrid.dataItem(this).weavingMachineCoreUuid);
  });
  theGrid.select().each(function () {
    theGrid.removeRow(this);
  });
  postData.append("weavingMachineCoreDelete", new Blob([JSON.stringify({
    "weavingMachineCoreUuids": uuidSet
  })], {
    type: "application/json"
  }));
