...
var purchaselistAjAX = $.ajax({
  url: "/api/machine/warping/purchase/company/list",
  method: "GET",
...
var subordinateAjAX = $.ajax({
  url: "/api/machine/warping/subordinate/company/list",
  method: "GET",
  success: function (data) {
...
$(async function () {
  ...
  var grid = new Grid("/machine/warping/", "grid,onlyEditColumn,check");
  grid.setSortField("purchaseDate", "desc");
  var kendoData = {
    id: "warpingMachineCoreNo",
    fields: {
  ...
  let saveData = {
    url: "/api/machine/warping/",
    saveEvent: function (e) {
      e.preventDefault();
      let theGrid = $("#warping").data("kendoGrid");
      var postData = new FormData();
      var data = new Object();
      console.log(theGrid.dataItem(theGrid.select()));

      data.warpingMachineCoreUuid = (theGrid.dataItem(theGrid.select()).warpingMachineCoreUuid) ? theGrid.dataItem(theGrid.select()).warpingMachineCoreUuid.trim() : 0;
      ...
      postData.append("warpingMachineCore", new Blob([JSON.stringify(data)], {
      ...
      $.ajax({
        url: "/api/machine/warping/",
        data: postData,
        method: (data.warpingMachineCoreUuid) ? "POST" : "PUT",
        contentType: false,
        processData: false,
        dataType: "json",
        success: function (data) {
  ...
  await grid.creatKendGrid("#warping", filterable, edit, false);
  $('body').on('click', '#warping .k-checkbox', function () {
    $("#trashBin").hide();

    if ($("#warping .k-grid-content tbody input:checkbox:checked").length) {
      $("#trashBin").show();
    }
  ...
  $('body').on('click', '#addbtn', function () {
    var grid = $("#warping").data("kendoGrid");
    grid.addRow();
    var row = $("#warping").find(".k-grid-edit-row");
    grid.select(row);
...
function confirmDel() {
  ...
  var uuidSet = [];
  var theGrid = $('#warping').data("kendoGrid");

  var postData = new FormData();
  theGrid.select().each(function () {
    uuidSet.push(theGrid.dataItem(this).warpingMachineCoreUuids);
  });
  theGrid.select().each(function () {
    theGrid.removeRow(this);
  });
  postData.append("warpingMachineCoreDelete", new Blob([JSON.stringify({
    "warpingMachineCoreUuids": uuidSet
  })], {
    type: "application/json"
  }));
