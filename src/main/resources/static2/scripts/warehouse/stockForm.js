$(async function () {
  var backUrl = "/page/warehouse/stock/";
  var apiUrl = "/api/warehouse/stock/template/" + uuid;
  let baseUrl = "/warehouse/stock/"
  let warehouseType = "yarn/"
  let activeType = "import/"
  $("#form").formWizard({
    id: "warehouseStockCoreUuid",
    url: apiUrl,
    mainData: "response",
    noData: "findnodata(backUrl)",
    customizeForm: "customizeForm",
  }); //formWizard
  gridUrl = baseUrl + warehouseType + activeType;
  creatGrid(gridUrl, "#form2");
  $("body").on("click", ".nav-link", function () {
    let type = $(this).attr("id");
    console.log(type);
    switch (type) {
      case "yarn":
        warehouseType = type + "/"
        break;
      case "beam":
        warehouseType = type + "/"
        break;
      case "buckle":
        warehouseType = type + "/"
        break;
      case "cloth":
        warehouseType = type + "/"
        break;
      case "import":
        activeType = type + "/"
        break;
      case "export":
        activeType = type + "/"
        break;
      default:
        break;
    }
    gridUrl = baseUrl + warehouseType + activeType;
    $("#form2").data("kendoGrid").destroy();
    $("#form2").empty();
    creatGrid(gridUrl, "#form2");

  }) //tab click event
  $('body').on('change', '[data-field ="actualExportDatetime"] .k-dropdown-operator input,[data-field ="actualImportDatetime"] .k-dropdown-operator input,[data-field ="exportActualTime"] .k-dropdown-operator input,[data-field ="importActualTime"] .k-dropdown-operator input', async function () {
    await this;
    let field = $(this).closest(".k-filtercell").attr("data-field")

    let html = '<div id="betweenDate"><span style="display:flex; justify-content:center;" class="dataRange"><input  class="start-date"/><span>&nbsp~&nbsp</span><input  class="end-date"/></span></div>';
    if ($(this).attr("aria-label") == "選擇間距") {
      kendo.culture("zh-TW");
      $(".k-widget .k-datepicker").hide();

      if ($("#betweenDate").length == 0) {
        let filterCell = $(this).parents(".k-filtercell");
        $(html).insertBefore($(".k-widget .k-datepicker"));

        $("#betweenDate").kendoDateRangePicker({
          culture: "zh-TW",
          format: "yyyy/MM/dd",
          change: function (e) {
            let startDate = this._range.start,
              endDate = this._range.end,
              dataSource = $("#form2").data("kendoGrid").dataSource;
            if (startDate & endDate) {
              let filter = {
                logic: "and",
                filters: []
              };
              filter.filters.push({
                field: field,
                operator: "gte",
                value: startDate
              });
              filter.filters.push({
                field: field,
                operator: "lte",
                value: endDate
              });
              dataSource.filter(filter);
            }

          }
        });
        $(".dataRange").next("span").remove();
        $(".dataRange").next("span").remove();
        $(".dataRange").next("span").remove();
      } else {
        $(".start-date").show()
        $(".end-date").show()
        $("#betweenDate").show();
      }
    } else {
      $(".k-widget .k-datepicker").show();
      var range = {
        start: "",
        end: ""
      };
      $("#betweenDate").data("kendoDateRangePicker").range(range);
      $("#betweenDate").hide();
    }
  })
});

async function creatGrid(gridUrl, id) {
  let filterable = {
    mode: "row",
    messages: {
      info: ""
    },
    operators: {
      string: {
        eq: "完全一致",
        contains: "包含",
      },
      number: {
        lte: "小於等於",
        eq: "等於",
        gte: "大於等於",
      },
      date: {
        gte: "之後",
        lte: "之前",
        eq: "相等",
        between: "選擇間距"
      }
    }
  };
  var grid = new Grid(gridUrl, "grid,noToolbar", "buckleCore", "buckleCore", uuid);
  await grid.initDataSource();
  await grid.creatKendGrid(id, filterable);
}


function customizeForm() {
  kendo.ui.progress($("#grid"), true);



  kendo.ui.progress($("#grid"), false);
}
