var rootApi = "/api/vendor/management";
var apiUrl = rootApi + "/template/";
var backUrl = "page/vendor/management";
          ...
          if (data.status) {
            ...
            setTimeout(function () {
              $(".drawCheck").hide().delay(460).queue(function () {
                location.replace("/page/vendor/management");
                $(this).dequeue();
              });
            }, 1000);
          } else {
...
function initMultiSelect(selector, api, value) {
  ...
  if (value) {
    $("[name='supplyChains']").data("kendoMultiSelect").value(value);
  }
}
...
function setSaveData() {
  ...
  companyManagementCoreView.supplyChains = $("[name='supplyChains']").data("kendoMultiSelect").value();
